...
public class Q4 {
	...
	// O(N), while this solution works, is far from optimal as it uses boxing
	// and Collections framework
	public static boolean isPermutationOfPalindrome(String input) {
		Map<Character, Boolean> counts = new HashMap<>();
		char[] str_array = input.toCharArray();
		for (char c : str_array) {
			if (!Character.isWhitespace(c)) {
				counts.compute(c, (k, v) -> (null == v) ? Boolean.TRUE : !v);
			}
		}

		int count = 0;
		for (Boolean each : counts.values()) {
			if (each)
				count++;
		}
